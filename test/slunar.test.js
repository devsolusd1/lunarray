...
describe("StakedLunarray (sLUNARRAY)", () => {
  async function setup() {
    ...
    const engine = await (await ethers.getContractFactory("BondEngine")).deploy(await token.getAddress(), ethers.ZeroAddress, await pm.getAddress(), await sv.getAddress(), ethers.ZeroAddress, 0, 200, guardian.address, treasury.address, HOUR, PARAMS);
    const s = await (await ethers.getContractFactory("StakedLunarray")).deploy(await token.getAddress(), await engine.getAddress());
  ...
  it("mints 1:1, stakes into the engine, streams ETH pro-rata and follows transfers", async () => {
    const c = await setup();
    expect(await c.s.name()).to.equal("Staked LUNARRAY");
    expect(await c.s.symbol()).to.equal("sLUNARRAY");
    ...
    // 4 ETH reaches the engine (protocol share): 50% to stakers -> 2 ETH for the sLUNARRAY pool
    ...
    // alice transfers 100 sLUNARRAY to bob: her accrued 1.5 stays hers; future rewards split 200/200

...
/// @title StakedLunarray (sLUNARRAY)
/// @notice Liquid staking receipt for the LUNARRAY BondEngine. Deposit LUNARRAY, receive sLUNARRAY 1:1; the
///         contract holds one staking position in the engine and streams the ETH it earns to sLUNARRAY holders
///         pro-rata. Transferable: rewards follow the balance. No owner, no fees, no pause. Unstake any time, 1:1.
contract StakedLunarray is ERC20, ReentrancyGuard {
    using SafeERC20 for IERC20;

    IERC20 public immutable lunarray;
    ...
    constructor(address lunarray_, address engine_) ERC20("Staked LUNARRAY", "sLUNARRAY") {
        lunarray = IERC20(lunarray_);
        engine = IBondEngineStaking(engine_);
        IERC20(lunarray_).forceApprove(engine_, type(uint256).max);
    }
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        _sync();
        lunarray.safeTransferFrom(msg.sender, address(this), amount);
        engine.stake(amount);
    ...
    function unstake(uint256 amount) external nonReentrant {
        ...
        lunarray.safeTransfer(msg.sender, amount);

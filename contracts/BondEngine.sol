...
import {IPonsCurve} from "./interfaces/IPons.sol";

/// @title BondEngine (Lunarray)
/// @notice Bond + stake engine for a fixed-supply token launched on Pons v2: live from the bonding curve, and on the
///         Uniswap v4 pool (ETH / token) after graduation.
///
///  Market   : before graduation the Pons curve is the market (price from its pricing reserves, buybacks through
///             `curve.buy`); once `curve.graduated()` the v4 pool takes over automatically (price from the StateView,
///             buybacks through the PoolManager). Nothing is redeployed or re-started at graduation.
///  Oracle   : one price sample per epoch (tokens per ETH); `target` = mean of the last `window` samples.
...
///             rewards, the rest to `ethReserve`. Every epoch `releaseBps` of the reserve buys token on the live
///             market and the tokens go to the crypt, funding bond payouts. A curve buy that is only partially
///             filled refunds the rest to the reserve; a curve buy that fails is skipped and the epoch still advances.
///  Stake    : stake token, earn ETH. No lock, no penalty.
///  Idle     : protocol funds (crypt tokens not owed to bonds, ETH reserve) can be swept to the treasury by the
///             guardian only after `IDLE_DELAY` with no bond outstanding. User stakes, staker rewards and owed
///             bonds can never be swept.
///  Safety   : no admin can move user funds. The guardian can only pause NEW entries for at most 7 days at a
///             time (unstake, exit, settle and claims never pause), propose parameter changes that execute
///             after a 48h timelock, and sweep idle protocol funds. Addresses, curve and pool are immutable.
contract BondEngine is IUnlockCallback, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // ------------------------------------------------------------------ immutables
    IERC20 public immutable token;
    IPonsCurve public immutable curve; // address(0) = no curve (pool only)
    ...
    uint64 public constant IDLE_DELAY = 30 days;
    ...
    uint256 internal refundInFlight; // ETH handed back by the curve during a partial buy
    ...
    uint64 public lastBondActivityAt; // last bond opened, settled or exited (idle clock)
    ...
    // ------------------------------------------------------------------ events / errors
    event Started(uint256 tokensPerEthQ96, uint8 source);
    event Poked(address indexed keeper, uint32 epoch, uint256 spot, uint256 target, uint256 tip);
    event Buyback(uint256 ethIn, uint256 tokensOut, uint8 source);
    event BuybackSkipped(uint256 ethIn, bytes reason);
    ...
    event IdleSwept(uint256 tokens, uint256 eth);
    ...
    constructor(
        ...
        if (token_ == address(0) || guardian_ == address(0) || treasury_ == address(0)) revert ZeroAddress();
        token = IERC20(token_);
        ...
        _validate(p);
        params = p;
        lastBondActivityAt = uint64(block.timestamp);
    }
    ...
    function onCurve() public view returns (bool) {
        return address(curve) != address(0) && !curve.graduated();
    }

    /// @return 0 = no live market, 1 = Pons curve (before graduation), 2 = Uniswap v4 pool
    function priceSource() public view returns (uint8) {
        if (onCurve()) return 1;
        (uint160 sqrtP, , , ) = stateView.getSlot0(poolId);
        return sqrtP == 0 ? 0 : 2;
    }

    /// @return tokensPerEthQ96 current market price as tokens per ETH in Q96 (0 if there is no live market)
    function spot() public view returns (uint256 tokensPerEthQ96) {
        if (onCurve()) {
            (uint256 q, uint256 t) = curve.getReserves();
            if (q == 0 || t == 0) return 0;
            return Math.mulDiv(t, 1 << 96, q);
        }
        (uint160 sqrtP, , , ) = stateView.getSlot0(poolId);
        if (sqrtP == 0) return 0;
    ...
    /// @return seconds since the last bond activity while nothing is owed to bonds (0 while bonds are outstanding)
    function idleFor() public view returns (uint256) {
        if (bondedOutstanding != 0) return 0;
        return block.timestamp - lastBondActivityAt;
    }
    ...
    // ------------------------------------------------------------------ lifecycle
    /// @notice Opens the engine as soon as there is a live market: the Pons curve right after launch, or the v4
    ///         pool. Permissionless.
    function start() external {
        if (started) revert AlreadyStarted();
        uint256 s = spot();
        if (s == 0) revert MarketNotLive();
        started = true;
        _pushSample(s);
        lastSampleAt = uint64(block.timestamp);
        emit Started(s, priceSource());
    }
    ...
    function poke() external nonReentrant {
        ...
        if (amt > 0) {
            ethReserve -= amt;
            (uint256 out, uint256 spent, uint8 src) = _buyback(amt);
            if (spent < amt) ethReserve += amt - spent; // partial fill or skipped: unspent ETH stays in the reserve
            if (out > 0) {
                crypt += out;
                totalBoughtBack += out;
                emit Buyback(spent, out, src);
            }
        }
    ...
    function settle(uint256 maxBonds) external nonReentrant returns (uint256 paid) {
        ...
        queueHead = i;
        if (paid > 0) lastBondActivityAt = uint64(block.timestamp);
    }
    ...
    /// @notice Moves idle protocol funds (the crypt and the ETH reserve) to the treasury. Only possible when no
    ///         bond is outstanding and none was opened, settled or exited for `IDLE_DELAY`. Staked tokens and
    ///         ETH owed to stakers are separate balances and are never touched.
    function sweepIdle() external onlyGuardian nonReentrant {
        if (bondedOutstanding != 0 || block.timestamp < lastBondActivityAt + IDLE_DELAY) revert NotIdle();
        uint256 t = crypt;
        uint256 e = ethReserve;
        crypt = 0;
        ethReserve = 0;
        lastBondActivityAt = uint64(block.timestamp);
        if (t > 0) token.safeTransfer(treasury, t);
        if (e > 0) {
    ...
    receive() external payable {
        if (msg.sender == address(curve)) {
            // refund of the unfilled part of a curve buy: accounted by poke(), never split
            refundInFlight += msg.value;
            return;
        }
    ...
    // ------------------------------------------------------------------ internals
    /// @dev Buys on the live market. Curve: partial fills are refunded (spent < ethIn) and a failing buy is skipped
    ///      (out = 0, spent = 0) so the epoch still advances. Pool: exact-input swap through the PoolManager.
    function _buyback(uint256 ethIn) internal returns (uint256 out, uint256 spent, uint8 src) {
        if (onCurve()) {
            refundInFlight = 0;
            try curve.buy{value: ethIn}(ethIn, 0, address(this)) returns (uint256 got) {
                out = got;
                spent = ethIn - refundInFlight;
            } catch (bytes memory reason) {
                emit BuybackSkipped(ethIn, reason);
            }
            refundInFlight = 0;
            return (out, spent, 1);
        }
